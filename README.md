...
インストールが完了したら、以下の手順で cursor-power をすぐに体験できます。

### ステップ 1: チュートリアルを実行

Cursor の Agent tab を開き、以下のコマンドを入力してください:

```
/tutorial
```

ステップバイステップのウォークスルーで、タスク登録から PR 作成までの一連の流れを体験できます。

### ステップ 2: 最初のタスクを作成

チュートリアル後、実際のタスクを作成してみましょう:

```
/task-plan
```

対話形式で仕様を詰めながらタスクを登録できます。背景・目的・対象ファイルを聞かれるので、順番に回答するだけで子エージェントが自動で実装を開始します。

### ステップ 3: 進捗を確認

子エージェントが作業を始めたら、進捗を確認します:
...
詳しい使い方は「[基本的な使い方](#基本的な使い方)」セクションを参照してください。
